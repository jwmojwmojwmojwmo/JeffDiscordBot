...
export async function execute(interaction) {
    ...
    if (last_daily && last_daily >= today) {
        ...
        return interaction.reply({ content: `You’ve already claimed your daily today! Next claim <t:${nextClaim.getTime() / 1000}:R>`, flags: MessageFlags.Ephemeral });
    }
    if (last_daily && last_daily >= yesterday) {
        user.daily_streak += 1;
    } else {
        user.daily_streak = 0;
        lost_daily = true;
    ...
    const msg = (lost_daily) ? "" : `\n\nYou have received an additional ${energyBonus} energy for your ${user.daily_streak} day streak!`;
    await user.save();
    console.log(`${user.username} (${user.userid}) claimed their daily`);
    await interaction.reply({ content: `Thanks for checking in! You have received your daily! +25 energy!${msg}`, flags: MessageFlags.Ephemeral }); // success
    if (lost_daily) {
